...
### CodePipeline を使った backend deploy

- CodeBuild role と CodePipeline role を分けた
- `buildspec-backend.yml` を使って backend image を build する
- CodeBuild から ECR に push する
- CodePipeline の Deploy stage から ECS service を更新する
- 結果として ECS の rolling update を観察できる状態にした
...
このプロジェクトでは、backend を題材に CodePipeline の最初の一歩を進め、ECS standard deployment の rolling update を確認するところまで進めた。
...
- CodePipeline による backend rolling update の学習開始
